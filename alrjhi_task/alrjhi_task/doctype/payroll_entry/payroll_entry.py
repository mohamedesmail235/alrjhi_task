...
from alrjhi_task.utils.global_functios import get_employee_cost_center
...
class PayrollEntry(Document):
	...
	def get_salary_components(self, component_type):
		...
		if salary_slips:
			salary_components = frappe.db.sql("""select salary_component, amount, parentfield,(select employee from `tabSalary Slip` where `name`=D.parent) Employee
		...
		if salary_components:
			...
			for item in salary_components:
				...
				if add_component_to_accrual_jv_entry:
					component_dict[item['salary_component']] = item

					accounts_detail.append(item)

			account_details = self.get_account(component_dict = accounts_detail)
	...
	def get_account(self, component_dict = None):
		...
		for item in component_dict:
			...
			inner_dict["salary_component"]= item["salary_component"]
			inner_dict["employee"]= item["Employee"]
			inner_dict["default_account"]=account
			inner_dict["amount"] = item["amount"]
			account_dict.append(inner_dict)
		...
		if earnings or deductions:
			...
			for item in earnings:
				...
				accounts.append({
						...
						"party_type": "Employee",
						"party": item["employee"],
						"debit_in_account_currency": flt(flt(item["amount"]), precision),
						"cost_center": get_employee_cost_center(item["employee"]) or self.cost_center,
			...
			for item in deductions:
				...
				accounts.append({
						...
						"party_type": "Employee",
						"party": item["employee"],
						"credit_in_account_currency": flt(flt(item["amount"]), precision),
						"cost_center": get_employee_cost_center(item["employee"]) or self.cost_center,
						"project": self.project
					})
