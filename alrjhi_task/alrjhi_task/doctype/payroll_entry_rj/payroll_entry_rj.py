...
class PayrollEntryRJ(Document):
		...
		if salary_components:
			...
			for item in salary_components:
				...
				if component_type == "earnings":
					is_flexible_benefit, only_tax_impact = frappe.db.get_value("Salary Component",
				...
				if add_component_to_accrual_jv_entry:
					component_dict[item['salary_component']] = component_dict.get(item['salary_component'], 0) + item[
						'amount']
		...
		if salary_slip_name_list and len(salary_slip_name_list) > 0:
			...
			for salary_slip_name in salary_slip_name_list:
				...
				for sal_detail in salary_slip.earnings:
					is_flexible_benefit, only_tax_impact, creat_separate_je, statistical_component = frappe.db.get_value(
						"Salary Component", sal_detail.salary_component,
						['is_flexible_benefit', 'only_tax_impact',
						 'create_separate_payment_entry_against_benefit_claim', 'statistical_component'])
				...
				for sal_detail in salary_slip.deductions:
					statistical_component = frappe.db.get_value("Salary Component", sal_detail.salary_component,
	...
	def get_count_employee_attendance(self, employee):
		...
		return marked_days
	def get_branch_cost_center(self):
		cost_center = frappe.db.sql("""
			select cost_center 
				from `tabBranch RJ`
				where name = %s
			
		""",self.branch,as_dict=True)
...
def get_payroll_entry_bank_entries(payroll_entry_name):
	journal_entries = frappe.db.sql(
		...
		'where reference_type="Payroll Entry RJ" '
...
def create_salary_slips_for_employees(employees, args, publish_progress=True):
	...
	payroll_entry = frappe.get_doc("Payroll Entry RJ", args.payroll_entry_rj)
...
def get_payroll_entries_for_jv(doctype, txt, searchfield, start, page_len, filters):
	return frappe.db.sql("""
			select name from `tabPayroll Entry RJ`
